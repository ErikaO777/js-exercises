...
// WebSocket接続、10秒間隔で送る
ws.addEventListener("open", () => {
  console.log("WebSocket connected");
});

// WebSocketから受信した盤面情報を、renderに渡して描画
...
canvas.addEventListener("click", function (evt) {
  ...
  const pos = { x: evt.clientX - rect.left, y: evt.clientY - rect.top }; // クリック位置

  const row = Math.floor(pos.y / RESOLUTION); // 行
  const col = Math.floor(pos.x / RESOLUTION); // 列
  grid[row][col] = !grid[row][col];
  sound.cloneNode().play();
  state.type = "toggle"; // セル情報を更新
  state.row = row;
  state.col = col;
