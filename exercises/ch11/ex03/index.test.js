...
describe("リトルエンディアンとビッグエンディアン", () => {
  ...
  test("リトルエンディアン→ビッグエンディアンへの変換", () => {
    ...
    const result3 = convertToBig(new Uint32Array([1000])); // 0x000003e8
    ...
    expect(Array.from(result3).map((x) => x.toString(10))).toEqual([
      "3892510720", // 0xc8030000
    ]);

    // リトルエンディアンで読むとE8 03 00 00
    // ビッグエンディアンにするため反転して00 00 03 E8
  });
  ...
  test("ビッグエンディアン→リトルエンディアンへの変換", () => {
    ...
    const result3 = convertToLittle(new Uint32Array([1000]));  // 0x000003e8
