export function convertToBig(array) {
  ...
  const newBuffer = new ArrayBuffer(array.buffer.byteLength); // 新しいバッファ
  const dataView = new DataView(newBuffer); // 新しいバッファのDataView
  const sourceView = new DataView(array.buffer); //

  for (let i = 0; i < array.length; i++) {
    // trueでリトルエンディアンで読み込み
    const value = sourceView.getUint32(i * 4, true);
    // falseでビッグエンディアンで書き込み
...
console.log(
  Array.from(convertToBig(new Uint32Array([0x12345678, 0x90abcdef]))).map((x) =>
    x.toString(16)
  )
);
console.log(
  Array.from(convertToBig(new Uint32Array([0x12345678]))).map((x) =>
    x.toString(16)
  )
); // 毎回新しい配列を作成
console.log("リトルエンディアンに変換");
console.log(
  Array.from(convertToLittle(new Uint32Array([0x12345678, 0x90abcdef]))).map(
    (x) => x.toString(16)
  )
);
console.log(
  Array.from(convertToLittle(new Uint32Array([0x12345678]))).map((x) =>
    x.toString(16)
  )
); // 毎回新しい配列を作成
